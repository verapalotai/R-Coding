...
```{r, log, eval=TRUE, message=FALSE, results='hide', warning=FALSE}
x>3
x>y
x==y
(x)
...
```{r, comp, eval=TRUE, message=FALSE, results='hide', warning=FALSE}
x > c(2,4,6)
```
...
```{r, viewvie, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
View(vienna)
```

Grouping by stars and counting observations within each group

```{r, stars, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r, dotplot, eval=TRUE, message=FALSE, warning=FALSE}
(p<-ggplot(by_stars) +
...
```{r, implib, eval=TRUE, message=FALSE, warning=FALSE, results='hide'}
library(rlang)
```

Set the path

```{r, pathset, eval=TRUE, message=FALSE, warning=FALSE, results='hide'}
dir <-  "D:/Egyetem/CEU/Coding_1/R-Coding/"
```

Location Folders

```{r, location, eval=TRUE, results='hide', warning=FALSE, message=FALSE}
...
```{r, join, eval=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r, viewvienna, eval=TRUE, results='hide', message=FALSE, warning=FALSE}
View(hotels_europe)
```
...
```{r, cityfilter, eval=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
```{r, londonstars, eval=TRUE, message=FALSE, warning=FALSE}
by_stars_dublin <- hotels_dublin %>% group_by(stars)

(stars_count_dublin <- by_stars_dublin %>% summarise(count = n()))
```

```{r, hist3, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r, viedist, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r, lnddist, eval=TRUE, message=FALSE, warning=FALSE}
ggplot(data = hotels_dublin, aes (x = distance, y = (..count..))) +
  geom_histogram(binwidth = 0.5, fill = "blue", size = 0.25, alpha = 0.8,  show.legend=F, na.rm =TRUE) +
  labs(x = "Distance to City Center (km)", y = "Frequency", title="The Distribution of Distance to City Centre in Vienna") +
  scale_x_continuous(limits = c(0, 14), breaks = seq(0, 14, by = 1))
```
...
```{r, vieprices2, eval=TRUE, message=FALSE, warning=FALSE}
...
```{r, lndprices, eval=TRUE, message=FALSE, warning=FALSE}
