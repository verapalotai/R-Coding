...
*location folders
global data_in   	"da_data_repo/sandp500/clean"
...
tsset date
tsline value
graph export "$data_out/output/sp500_daily.png", as(png) replace

sort date
gen pct_return=(value-value[_n-1])/value[_n-1]*100
tsline pct_return
graph export "$data_out/output/sp500_return_daily.png", as(png) replace


collapse (mean) value, by (year )
tsset year
tsline value
graph export "$data_out/output/sp500_year.png", as(png) replace
sort year
gen pct_return=(value-value[_n-1])/value[_n-1]*100
tsline pct_return
graph export "$data_out/output/sp500_return_year.png", as(png) replace
...
graph export "$data_out/output/returns_histogram.png",replace

keep date pct_return
save "$data_out/temp/sp500_pctreturns.dta", replace
...
use "$temp/repsamples450",replace
 keep loss$loss
 rename loss$loss loss_n450
merge 1:1 _n using "$temp/repsamples900", nogen
...
line y450 x450 , lw(thick) lc(green) lp(shortdash) ///
 ...
 ytitle("Frequency") xtitle("percent of days with losses above 5%") ///
  xlab(0(0.5)2.5) xline(0.5) ylab(0) legend(rows(1))
graph export "$data_out/output/resample_densities.png",replace
*/
