...
# set the path to the textbook master folder!
dir <-  "C:/Users/..."
...
histprice_Vienna1 <- ggplot(data =  vienna_cut, aes (x = price)) +
  geom_histogram(binwidth = 20,  fill = color[1], size = 0.5, alpha = 0.8,  show.legend=F, na.rm =TRUE) +
  labs(x = "Price", y = "Frequency") +
  scale_x_continuous(limits = c(0,1100), breaks = seq(0, 1100, by = 100)) +
  theme_bg() +
  background_grid(major = "xy", minor = "y")
histprice_Vienna1
ggsave(paste0(output, "histprice_Vienna_E_R.png"), width=12, height=9, units = "cm", dpi = 1200)
cairo_ps(filename = paste0(output, "histprice_Vienna_E_R.eps"),
         width = mywidth_large, height = myheight_large, pointsize = 12,
         fallback_resolution = 1200)
print(histprice_Vienna1)
dev.off()
